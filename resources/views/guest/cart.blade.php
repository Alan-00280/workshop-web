    ...
    <form action="{{ route('cart-put') }}" method="POST">
        ...
        <div class="row pt-2">
            ...
            <div class="col-12 mb-4 text-center text-md-start">
                <h1 class="display-5 fw-bold text-primary"><i class="fa-solid fa-cart-shopping me-3"></i>Keranjang Belanja
                </h1>
            ...
            <div class="col-lg-8 mb-4">
                <div id="cart-items-container">
                    @forelse ($cart_items as $index => $item)
                        <div class="card border-0 bg-white cart-item-card shadow-sm mb-3 cart-item-row"
                            data-price="{{ $item->menu->harga }}"
                            data-id="{{ $item->idkeranjang ?? ($item->menu->idkeranjang ?? '') }}">
                            <input type="hidden" name="keranjangs[{{ $index }}][idkeranjang]" value="{{ $item->idkeranjang }}">
                            <input type="hidden" name="keranjangs[{{ $index }}][idmenu]" value="{{ $item->idmenu }}">
                            <div class="card-body p-3 p-md-4">
                                <div class="row align-items-center">

                                    <!-- Image -->
                                    <div class="col-12 col-md-2 text-center text-md-start mb-3 mb-md-0">
                                        <img src="{{ $item->menu->path_gambar }}" alt="{{ $item->menu->nama_menu }}"
                                            class="img-fluid border-3 border-white shadow-sm"
                                            style="width: 110px; height: 110px; object-fit: cover; border-radius: 15px;">
                                    </div>

                                    <!-- Details -->
                                    <div class="col-12 col-md-4 text-center text-md-start mb-3 mb-md-0 px-md-3">
                                        <h5 class="fw-bold mb-1 text-dark">{{ $item->menu->nama_menu }}</h5>
                                        <div class="mb-2">
                                            <span class="badge bg-light text-secondary border"><i
                                                    class="fa-solid fa-store me-1"></i>{{ $item->menu->vendor->nama_vendor }}</span>
                                        </div>
                                        <h6 class="text-primary fw-bold mb-0">Rp
                                            {{ number_format($item->menu->harga, 0, ',', '.') }}</h6>
                                    </div>
                                    ...
                                    <div class="col-6 col-md-3 text-center mb-2 mb-md-0">
                                        <div class="d-flex justify-content-center align-items-center">
                                            <button type="button"
                                                class="btn btn-sm btn-outline-secondary rounded-circle btn-minus"
                                                style="width: 30px; height: 30px; padding: 0;"><i class="fa-solid fa-minus"
                                                    style="font-size: 0.8rem;"></i></button>
                                            <input type="number" name="keranjangs[{{ $index }}][quantity]"
                                                class="form-control form-control-sm mx-2 qty-input"
                                                value="{{ $item->quantity }}" min="1">
                                            <button type="button"
                                                class="btn btn-sm btn-outline-secondary rounded-circle btn-plus"
                                                style="width: 30px; height: 30px; padding: 0;"><i class="fa-solid fa-plus"
                                                    style="font-size: 0.8rem;"></i></button>
                                        </div>
                                    </div>
                                    ...
                                    <!-- Total Price & Actions -->
                                    <div
                                        class="col-6 col-md-3 text-end d-flex flex-column align-items-end justify-content-center">
                                        <p class="fw-bold fs-5 mb-2 text-dark item-total-price">Rp
                                            {{ number_format($item->menu->harga * $item->quantity, 0, ',', '.') }}</p>
                                        <button type="button"
                                            class="btn btn-sm btn-outline-danger shadow-sm rounded-pill px-3 btn-remove-item"><i
                                                class="fa-regular fa-trash-can me-1"></i>Hapus</button>
                                    </div>
                ...
                <div class="alert alert-light text-center py-5 shadow-sm border-0 rounded-4 mt-3" id="dynamic-empty-alert"
                    style="display: {{ $cart_items->isEmpty() ? 'block' : 'none' }};">
                    ...
                    <a href="{{ route('products-page') }}" class="btn btn-primary rounded-pill px-4 mt-2 shadow-sm">Belanja Sekarang</a>
                </div>
            </div>
            ...
            <div class="col-lg-4 cart-summary-container">
                <div class="cart-summary border-0">
                    ...
                    <div class="d-flex justify-content-between mb-3 text-secondary">
                        ...
                        <span class="summary-subtotal-price summary-total-price">Rp
                    ...
                    <div class="d-flex justify-content-between mb-4 mt-3">
                        ...
                        <h5 class="fw-bold text-primary mb-0 summary-total-price">Rp
                            {{ number_format($total_harga, 0, ',', '.') }}</h5>
                    </div>
                    <button type="submit"
                        class="btn btn-primary w-100 rounded-pill py-3 shadow-sm fw-bold btn-checkout {{ $cart_items->isEmpty() ? 'disabled' : '' }}">Beli
                        Sekarang (<span class="btn-checkout-count">{{ $cart_items->sum('quantity') }}</span>)</button>
                </div>
            </div>
        </div>
    </form>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '.btn-remove-item', function () {
                ...
                Swal.fire({
                    title: "Apakah Anda yakin ingin menghapus produk ini dari keranjang?",
                    showDenyButton: true,
