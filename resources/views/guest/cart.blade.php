{{-- @php
    ...
    $total_harga = $cart_items->sum(function($item) {
        return $item->menu->harga * $item->menu->quantity;
    });
@endphp --}}
    ...
    <form action="{{ route('cart-put') }}" method="POST">
        ...
        <div class="col-lg-8 mb-4">
            <div id="cart-items-container">
                @forelse ($cart_items as $index => $item)
                    <div class="card border-0 bg-white cart-item-card shadow-sm mb-3 cart-item-row" data-price="{{ $item->menu->harga }}" data-id="{{ $item->idkeranjang ?? ($item->menu->idkeranjang ?? '') }}">
                        ...
                        <div class="card-body p-3 p-md-4">
                            <div class="row align-items-center">
                                ...
                                <div class="col-12 col-md-2 text-center text-md-start mb-3 mb-md-0">
                                    <img src="{{ $item->menu->path_gambar }}" alt="{{ $item->menu->nama_menu }}" class="img-fluid border-3 border-white shadow-sm" style="width: 110px; height: 110px; object-fit: cover; border-radius: 15px;">
                                </div>
                                ...
                                <div class="col-12 col-md-4 text-center text-md-start mb-3 mb-md-0 px-md-3">
                                    ...
                                    <div class="mb-2">
                                        <span class="badge bg-light text-secondary border"><i class="fa-solid fa-store me-1"></i>{{ $item->menu->vendor->nama_vendor }}</span>
                                    </div>
                                    <h6 class="text-primary fw-bold mb-0">Rp {{ number_format($item->menu->harga, 0, ',', '.') }}</h6>
                                </div>
                                ...
                                <div class="col-6 col-md-3 text-center mb-2 mb-md-0">
                                    <div class="d-flex justify-content-center align-items-center">
                                        ...
                                        <input type="number" name="keranjangs[{{ $index }}][quantity]" class="form-control form-control-sm mx-2 qty-input" value="{{ $item->quantity }}" min="1">
                                ...
                                <div class="col-6 col-md-3 text-end d-flex flex-column align-items-end justify-content-center">
                                    <p class="fw-bold fs-5 mb-2 text-dark item-total-price">Rp {{ number_format($item->menu->harga * $item->quantity, 0, ',', '.') }}</p>
        ...
        <div class="col-lg-4 cart-summary-container">
            <div class="cart-summary border-0">
                ...
                <button type="submit" class="btn btn-primary w-100 rounded-pill py-3 shadow-sm fw-bold btn-checkout {{ $cart_items->isEmpty() ? 'disabled' : '' }}">Beli Sekarang (<span class="btn-checkout-count">{{ $cart_items->sum('quantity') }}</span>)</button>
            </div>
        </div>
        </div>
    </form>
